...

{
    ...
    for(i=2;i<n_y_inlet;i++)
    {
        ...
        F_e=dy*u_face(i,j);
        F_w=dy*u_face(i,j - 1);  //inlet velocity
        F_n=dx*v_face(i - 1,j);
        F_s=dx*v_face(i,j);
        ...
        source_x[n]=0.5*alpha_uv*(p(i,j) - p(i,j + 1))*dy + (1-alpha_uv)*A_p(i,j)*u_star(i,j) + alpha_uv*velocity*(2*D_w + std::max(0.0,F_w));  // P_o - 0.5(P_o+P_e)
    ...
    for(i=2;i<n_y;i++)
    {
        ...
        A_n(i,j)=D_n + std::max(0.0,-F_n);
        A_s(i,j)=D_s + std::max(0.0,F_s);
        A_p(i,j)= D_w + D_n + D_s + std::max(0.0,F_e) - std::max(0.0,-F_w) + std::max(0.0,-F_s) + std::max(0.0,F_n); //viscous effects neglected for outlet

        source_x[n]=alpha_uv*(0.5*(p(i,j-1) + p(i,j))-p_out)*dy + (1-alpha_uv)*A_p(i,j)*u_star(i,j);   //0.5(P_w+P_o)-P_o
        source_y[n]=0.5*alpha_uv*(p(i + 1,j) - p(i - 1,j))*dx + (1-alpha_uv)*A_p(i,j)*v_star(i,j);
    ...
    A_w(i,j)=D_w + std::max(0.0,F_w);
    A_s(i,j)=D_s + std::max(0.0,F_s);
    A_p(i,j)= D_w + 2*D_n + D_s + std::max(0.0,F_e) + std::max(0.0,-F_w) + std::max(0.0,-F_s) + std::max(0.0,F_n);
    ...
    A_w(i,j)=D_w + std::max(0.0,F_w);
    A_n(i,j)=D_n + std::max(0.0,-F_n);
    A_p(i,j)=D_w + D_n + 2*D_s + std::max(0.0,F_e) + std::max(0.0,-F_w) + std::max(0.0,-F_s) + std::max(0.0,F_n);
...
void pressure_correction_link_coefficients(MatrixXd &u,MatrixXd &u_face,MatrixXd &v_face,MatrixXd &Ap_p,MatrixXd &Ap_e,MatrixXd &Ap_w,MatrixXd &Ap_n,MatrixXd &Ap_s,VectorXd &source_p,MatrixXd &A_p)
{
    ...
    for(i=2;i<n_y;i++)
    {
        ...
        Ap_w(i,j)=0.5*alpha_uv*(1/A_p(i,j) + 1/A_p(i,j - 1))*(dy*dy) - 0.5*alpha_uv*(dy*dy)/A_p(i,j);
        Ap_n(i,j)=0.5*alpha_uv*(1/A_p(i,j) + 1/A_p(i - 1,j))*(dx*dx);
        Ap_s(i,j)=0.5*alpha_uv*(1/A_p(i,j) + 1/A_p(i + 1,j))*(dx*dx);
        Ap_p(i,j)= Ap_n(i,j) + Ap_s(i,j) + 0.5*alpha_uv*(1/A_p(i,j) + 1/A_p(i,j - 1))*(dy*dy) + 0.5*alpha_uv*(dy*dy)/A_p(i,j);
...
void post_processing(MatrixXd u_star,MatrixXd v_star,MatrixXd p_star)
{
    ...
    //Saving outputs for post-processing
     ...
     outFile.open("/Users/deep/Desktop/bfs/u.dat");
     outFile << u_star;
     outFile.close();

     outFile.open("/Users/deep/Desktop/bfs/v.dat");
     outFile << v_star;
     outFile.close();

     outFile.open("/Users/deep/Desktop/bfs/p.dat");
     outFile << p_star;
     outFile.close();

     outFile.open("/Users/deep/Desktop/bfs/x.dat");
     outFile << x;
     outFile.close();

     outFile.open("/Users/deep/Desktop/bfs/y.dat");
     outFile << y;
     outFile.close();
...
//Boundary conditions
u.block(1,0,n_y_inlet,1)=MatrixXd::Constant(n_y_inlet,1,1.0);
u_star.block(1,0,n_y_inlet,1)=MatrixXd::Constant(n_y_inlet,1,1.0);
u_face.block(1,0,n_y_inlet,1)=MatrixXd::Constant(n_y_inlet,1,1.0);
...
for(int n=1;n<=max_outer_iterations;n++)
{
    ...
    u.block(0,n_x+1,n_y+2,1)=u.block(0,n_x,n_y+2,1);
    
    solve(A_momentum,source_y,v,l2_norm_y,epsilon_uv);

    face_velocity(u,v,u_face,v_face,p,A_p);

    u_face.block(0,n_x,n_y+1,1)=u.block(0,n_x+1,n_y+1,1);
    ...
    //outlet boundary condition
    u_star.block(0,n_x+1,n_y+2,1)=u_star.block(0,n_x,n_y+2,1);
